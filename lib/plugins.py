...
def init_plugins(self):
    ...
    if __builtin__.use_local_modules:
        ...
        imp.load_module('shuttle_plugins', fp, pathname, description)
        plugin_modules = map(lambda name: imp.load_source('shuttle_plugins.'+name, os.path.join(pathname,name+'.py')), plugin_names)
    else:
        import shuttle_plugins
        plugin_names = [name for a, name, b in pkgutil.iter_modules(shuttle_plugins.__path__)]
        plugin_modules = [ __import__('shuttle_plugins.'+name, fromlist=['shuttle_plugins']) for name in plugin_names]
